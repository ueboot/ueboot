            ...
            getRuleValidate(columns) {
                ...
                columns.forEach((c) => {
                    ...
                    if (c.required && !c.rules) {
                        ...
                        // 级联框，时间或日期范围
                        if (this.includes(['cascader', 'checkbox'],c.type)) {
                            rule['type'] = 'array'
                        } else if (this.includes(['datetime', 'date', 'time', 'month', 'year'],c.type)) {
                            rule['type'] = 'date'
                        } else if (this.includes(['daterange', 'datetimerange'],c.type)) {
                            rule['type'] = 'object'
                        } else if (this.includes(['treeSelect', 'number'],c.type)) {
                            rule['type'] = 'number'
                        } else {
                            rule['type'] = 'string'
                        }
