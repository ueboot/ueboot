  ...
  <div>
        ...
        <i-col :span="24">
          <Row v-for="(row,index1) in formGrid.toolbar.superFilter.rows" :key="'row'+index1">
            <i-col v-for="(item,index2) in row" :key="'superfilter'+index2"
                   :span="24/formGrid.toolbar.superFilter.colNumber">
              <Form-item :label="item.label" :prop="item.name">
                ...
                <i-input v-model="queryParams[item.name]" :type="item.type" :placeholder="item.placeholder"
                ...
                <i-select v-if="item.type === 'select'" :clearable="item.clearable" :filterable="item.filterable"
                ...
                >
                  <Option v-for="(option,index) in item.items" :value="option.value" :key="'o'+index">{{ option.name }}
                  </Option>
                </i-select>
                ...
                <template v-else-if="item.type==='treeSelect'">
                  <u-tree-select :tree="item.tree" :refName="item.name"
                                 :fixed="item.fixed" :size="item.size" :maxHeight="item.maxHeight" :sort="item.sort"
                ...
                <template v-if="item.type === 'queryButton' && formGrid.toolbar.superFilter.submit">
                  ...
                  <Button v-if="formGrid.toolbar.superFilter.reset"
                          ...
                          :long="formGrid.toolbar.superFilter.reset.long">{{formGrid.toolbar.superFilter.reset.label}}
                  </Button>
                </template>
              </Form-item>
            </i-col>
    ...
    <Row :gutter="5" type="flex" :justify="formGrid.toolbar.justify" v-if="formGrid.toolbar.show">
      ...
      <i-col v-if="formGrid.toolbar.delete &&formGrid.toolbar.delete.show">
        <Button type="primary" icon="md-trash" @click="batchDelete" :loading="formGrid.toolbar.delete.loading" ghost>
      ...
      <i-col v-for="(button,index) in formGrid.toolbar.button" :key="index" v-if="formGrid.toolbar.button">
        <Button :type="button.theme" :icon="button.icon" :custom-icon="button.customIcon"
                ...
                :disabled="button.disabled" :to="button.to" :replace="button.replace" :target="button.target"
    ...
    <Modal
      ...
      <div slot="footer">
        ...
        <Button type="primary" @click="handleReset()" style="margin-left: 8px" :disabled="formGrid.form.loading" ghost>
          重置
        </Button>
            ...
            <Form-item :label="item.label" :prop="item.name" v-if="item.show">
              ...
              <template v-else-if="item.type==='date'">
                <DatePicker type="date" :placeholder="item.placeholder?item.placeholder:'选择日期'"
                            ...
                            :format="item.format?item.format:'yyyy-MM-dd'" :readonly="item.readonly" readonly
                ></DatePicker>
              </template>
