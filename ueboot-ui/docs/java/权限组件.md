...
- 使用npm 命令安装依赖 `npm install ueboot-shiro --save-dev`
- 配置vue路由，设置登录页面、主框架页面
  > 登录页面和主框架页面也可以自己写，只需调用后台相应的接口即可（参见服务端API接口）
  
```javascript
// ueboot-shiro框架
import ShiroMain from 'ueboot-shiro/src/views/Main'
import ShiroLogin from 'ueboot-shiro/src/views/Login'
import ShiroUser from 'ueboot-shiro/src/views/User'
import ShiroRole from 'ueboot-shiro/src/views/Role'
import ShiroResources from 'ueboot-shiro/src/views/Resources'

let Login = ShiroLogin
let Main = ShiroMain
...
## 界面参数配置
 ...
 ```javascript
import UebootShiroConfig from 'ueboot-shiro/src/ueboot-shiro'
/* 设置登录后的主界面相关配置 */
UebootShiroConfig.setConfig({
